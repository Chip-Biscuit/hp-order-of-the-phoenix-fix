...
HexEdit CreateHexEditFromResolution(int resolutionIndex) {
    ...
    case 11:
        ...
        break;
    case 12:
        edit.modified = { 0x80, 0x00, 0x05, 0x00, 0x00, 0xC7, 0x45, 0x84, 0x20, 0x03 };  // 1280 x 800
        edit.offset = 0;
        break;
    default:
...
void PerformHexEdit4() {
    ...
    float predefinedFloats[] = { 0.02f, 0.024f, 0.026f };
...
// Function to perform the hex edit
void PerformHexEdit6(LPBYTE lpAddress, DWORD moduleSize) {
    ...
    // Define the edits
    std::vector<HexEdit6> edits6 = {
        // FPS animations 30fps
        { { 0x02, 0x00, 0x00, 0x00, 0xE8, 0x8F, 0xE6, 0x75 }, { 0x01 }, 0 },
    };

    // Iterate through the edits
    for (const auto& edit6 : edits6) {
        // Search for the pattern in memory
        for (DWORD i = 0; i < moduleSize - edit6.pattern.size(); ++i) {
            if (memcmp(lpAddress + i, edit6.pattern.data(), edit6.pattern.size()) == 0) {
                // Pattern found in memory
                DX_PRINT("Pattern found in memory.")

                    // Modify memory
                    LPVOID lpAddressToWrite = lpAddress + i + edit6.offset;
                SIZE_T numberOfBytesWritten;
                DWORD oldProtect;
                if (!VirtualProtectEx(GetCurrentProcess(), lpAddressToWrite, edit6.newValue.size(), PAGE_EXECUTE_READWRITE, &oldProtect)) {
                    DX_ERROR("Failed to change memory protection.")
                        return;
                }

                BOOL result = WriteProcessMemory(GetCurrentProcess(), lpAddressToWrite, edit6.newValue.data(), edit6.newValue.size(), &numberOfBytesWritten);
                if (!result || numberOfBytesWritten != edit6.newValue.size()) {
                ...
                // Restore original protection
                DWORD dummy;
                if (!VirtualProtectEx(GetCurrentProcess(), lpAddressToWrite, edit6.newValue.size(), oldProtect, &dummy)) {
                    DX_ERROR("Failed to restore memory protection.")
                        return;
                }
...
void PerformHexEdits6() {
    ...
    PerformHexEdit6(lpAddress, moduleSize);
}
